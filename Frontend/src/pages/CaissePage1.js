...
// Fonction principale du composant Caisse
function CaissePage() {
...
const [dialogFilterOpen, setDialogFilterOpen] = useState(false);
  ...
  const [historiqueDuJour, setHistoriqueDuJour] = useState([]);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        setLoading(true);
        const [articlesResponse, clientsResponse, caisseStatusResponse] = await Promise.all([
          apiClient.get('api/articles'),
          apiClient.get('api/clients'),
          apiClient.get('api/caisse/status') // Endpoint à créer côté backend
        ]);
        
        setArticles(articlesResponse.data);
        setClients(clientsResponse.data);
        
        // Supposons que l'API renvoie le statut de la caisse
        ...
        if (caisse.isOpen) {
          // Charger les transactions du jour si la caisse est ouverte
          const transactionsResponse = await apiClient.get(`api/caisse/transactions`);
          setTransactions(transactionsResponse.data);
          
          // Charger le journal de caisse
          const journalResponse = await apiClient.get(`api/caisse/journal`);
          setJournalCaisse(journalResponse.data);
  ...
  const handleOuvertureCaisse = async () => {
    try {
      ...
      const response = await apiClient.post('api/caisse/ouverture', {
        soldeInitial: caisseStatus.soldeInitial,
      ...
      setCaisseStatus({
        isOpen: true,
        soldeInitial: caisseStatus.soldeInitial,
        soldeCourant: caisseStatus.soldeInitial,
  ...
  const handleFermetureCaisse = async () => {
    try {
      ...
      const response = await apiClient.post('api/caisse/fermeture', {
        soldeFinal: caisseStatus.soldeCourant,
        montantEspeces: reconciliation.montantEspeces,
        montantCheques: reconciliation.montantCheques,
        montantCartes: reconciliation.montantCartes,
  ...
  const handleFinalizeSale = async () => {
    try {
      ...
      // Récupérer les IDs d'entreprise et utilisateur
      ...
      // Utiliser le service de vente existant
      await createVente(venteEnCours, { entrepriseId, userId });
      
      // Mettre à jour le solde de la caisse (pour les paiements en espèces)
      if (venteEnCours.modePaiement === 'especes') {
        const newSoldeCourant = caisseStatus.soldeCourant + venteEnCours.totalTTC;
        
        setCaisseStatus(prev => ({
          ...prev,
          soldeCourant: newSoldeCourant
        }));
        
        // Enregistrer le mouvement dans le journal de caisse
        const newMouvement = {
          type: 'ENCAISSEMENT',
          montant: venteEnCours.totalTTC,
          modePaiement: 'ESPECES',
          date: new Date().toISOString(),
          description: `Vente #${Date.now()}`
        };
        
        setJournalCaisse(prev => [...prev, newMouvement]);
      }
  ...
  try {
    ...
    // Appel à l'API pour récupérer l'historique des ventes
    const historiqueResponse = await apiClient.get('api/ventes/historique');
    setHistoriqueVentes(historiqueResponse.data);
    // Filtrer par défaut sur la journée actuelle
    filtrerHistorique(historiqueResponse.data);
  } catch (error) {
    console.error("Erreur lors du chargement de l'historique des ventes:", error);
    setSnackbar({
      open: true,
      message: "Erreur lors du chargement de l'historique des ventes",
      severity: 'error'
    });
  } finally {
            ...
            <Paper sx={{ p: 2, mb: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                ...
                <Button
                  ...
                  onClick={() => setDialogues({ ...dialogues, clientSelection: true })}
                >
...
<Dialog open={dialogues.clientSelection} onClose={() => setDialogues({...dialogues, clientSelection: false})}>
  <DialogTitle>Sélectionner un client</DialogTitle>
  <DialogContent>
    <Button 
      fullWidth 
      variant="outlined" 
      onClick={handleClientComptoir}
      sx={{ mb: 2 }}
    >
      Client comptoir
    </Button>
    ...
    <List>
      {clients.filter(c => c.nom !== 'Client comptoir').map(client => (
        <ListItem button key={client._id} onClick={() => handleSelectClient(client)}>
          <ListItemText 
            primary={client.nom} 
