// client/src/components/achat/ArticlesStep.js - Étape 2: Sélection des articles pour un achat
...
function ArticlesStep({ achatData, updateAchatData }) {
  // État du composant
  ...
  useEffect(() => {
    calculateTotals();
  }, [achatData.articles, achatData.remise]);
  ...
  const calculateTotals = () => {
    const articles = achatData.articles || [];
    ...
    // Remise globale
    const remiseGlobale = sousTotal * (achatData.remise / 100);
  ...
  const handleSelectArticle = (article) => {
    ...
    setCurrentArticle({
      ...currentArticle,
      article,
      prixUnitaire: article.prixAchatHT || 0, // Utiliser le prix d'achat au lieu du prix de vente
  ...
  const handleCurrentArticleChange = (e) => {
    ...
    if (['quantite', 'prixUnitaire', 'remise', 'tva'].includes(name)) {
      ...
      const tva = name === 'tva' ? numericValue : updatedArticle.tva;
  ...
  const handleAddArticle = () => {
    ...
    if (editIndex >= 0) {
      ...
      updatedArticles = [...achatData.articles];
      updatedArticles[editIndex] = newArticleItem;
    } else {
      // Ajout d'un nouvel article
      updatedArticles = [...(achatData.articles || []), newArticleItem];
    }
    
    updateAchatData({ articles: updatedArticles });
  ...
  const handleEditArticle = (index) => {
    const item = achatData.articles[index];
  ...
  const handleDeleteArticle = (index) => {
    const updatedArticles = [...achatData.articles];
  ...
  return (
    <Box>
      <Typography variant="h6" gutterBottom sx={{ mb: 3 }}>
        Sélection des articles pour l'achat
      </Typography>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {achatData.articles && achatData.articles.length > 0 ? (
                achatData.articles.map((item, index) => (
                  <TableRow key={index}>
                  ...
                  <li {...props}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {option.prixAchatHT?.toFixed(2) || "0.00"} TND
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <Typography variant="body2" color="text.secondary">
